"""Test that importing agentrt.sdk completes within a reasonable time.
...
# Upper bound (seconds) for `import agentrt.sdk` in a cold process.
...
def _measure_import_time_seconds() -> float:
    """Return wall-clock seconds to `import agentrt.sdk` in a subprocess."""
    code = (
        "import time; "
...
def test_import_openhands_sdk_time():
    """Import of agentrt.sdk must complete under the time limit."""
    times = [_measure_import_time_seconds() for _ in range(_ITERATIONS)]
    avg = sum(times) / len(times)
    print(
        f"\n[import-perf] agentrt.sdk import times (s): {[f'{t:.3f}' for t in times]}"
    )
