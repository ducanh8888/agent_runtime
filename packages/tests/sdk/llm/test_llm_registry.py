...
class TestLLMRegistry(unittest.TestCase):
    ...
    def test_subscribe_and_notify(self):
        ...
        with patch(
            "agentrt.sdk.llm.llm_registry.RegistryEvent"
...
def test_llm_registry_notify_exception_handling():
    ...
    with patch("agentrt.sdk.llm.llm_registry.logger") as mock_logger:
        # Create a mock event
        mock_event = Mock()
...
def test_llm_registry_list_usage_ids():
    ...
    # Mock the RegistryEvent to avoid LLM attribute access
    with patch("agentrt.sdk.llm.llm_registry.RegistryEvent") as mock_registry_event:
        mock_registry_event.return_value = Mock()
