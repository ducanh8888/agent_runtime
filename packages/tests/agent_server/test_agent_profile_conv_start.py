...
async def _start_from_profile(
    ...
    async def capture_start(stored, **kwargs):
        ...
        event_service.stored = MagicMock(
            ...
            client_tools=[],
            title=None,
            # Added by H4/H5; the projection reads them, and a fake missing one
            # fails loudly rather than silently diverging from the real record.
            workspace_mode="shared",
            workspace_resolved_sha=None,
            workspace_prepared_at=None,
            workspace_capture=None,
            workspace_capture_detail=None,
