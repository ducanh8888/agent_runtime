...
default install dir is redirected), so nothing touches the real ~/.agentrt.
"""
...
def client(tmp_path: Path, monkeypatch) -> TestClient:
    ...
    store.mkdir()
    monkeypatch.setattr(
        "agentrt.sdk.plugin.installed.DEFAULT_INSTALLED_PLUGINS_DIR", store
    )
    app = FastAPI()
...
def test_list_available_returns_plugin_summaries(
    ...
    monkeypatch.setattr(
        "agentrt.sdk.plugin.load_available_plugins",
