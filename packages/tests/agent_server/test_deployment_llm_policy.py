"""Unit tests for the deployment-only LLM policy contract (H0).

The policy is opt-in on ``Config.deployment_llm_policy``. These tests pin the
validation rules the agent-server applies after every conversation creation
form has resolved: direct https DeepSeek, model ``deepseek-flash``, Chat
...
def test_direct_https_flash_passes(model: str) -> None:
    ...
    assert llm_satisfies_policy(llm, DeploymentLLMPolicy())
    enforce_agent_policy(_agent(llm), DeploymentLLMPolicy())
...
def test_http_endpoint_is_rejected() -> None:
    ...
    violations = llm_policy_violations(llm, DeploymentLLMPolicy())
...
def test_missing_endpoint_is_rejected() -> None:
    violations = llm_policy_violations(
        _direct_llm(base_url=None), DeploymentLLMPolicy()
    )
...
def test_extra_body_rejection_never_echoes_hostile_values() -> None:
    ...
    for extra_body in (
        ...
        violations = llm_policy_violations(
            _direct_llm(litellm_extra_body=extra_body), DeploymentLLMPolicy()
        )
...
def test_each_contract_axis_is_enforced(
    ...
    assert llm_policy_violations(llm, DeploymentLLMPolicy())
...
def test_switch_llm_default_tool_is_refused() -> None:
    ...
    with pytest.raises(DeploymentPolicyError, match="switch_llm"):
        enforce_agent_policy(agent, DeploymentLLMPolicy())
