...
from agentrt.agent_server.sockets import (
...
async def test_no_auth_configured_accepts_immediately():
    ...
    with patch("agentrt.agent_server.sockets.get_default_config") as mock_config:
...
async def test_first_message_auth_timeout():
    ...
    with (
        ...
        patch(
            "agentrt.agent_server.sockets._FIRST_MESSAGE_AUTH_TIMEOUT_SECONDS", 0.05
        ),
    ):
        mock_config.return_value.session_api_keys = ["sk-oh-valid"]
        result = await _accept_authenticated_websocket(ws, session_api_key=None)
...
async def test_bash_socket_ignores_redundant_auth_before_command(
    ...
    from agentrt.agent_server.sockets import bash_events_socket
    ...
    with (
        patch(
            "agentrt.agent_server.sockets.bash_event_service",
            mock_bash_service,
        ),
        patch("agentrt.agent_server.sockets.get_default_config") as mock_config,
    ):
