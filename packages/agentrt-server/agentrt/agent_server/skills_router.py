...
from agentrt.sdk.skills.utils import SKILL_NAME_PATTERN
...
class SkillsRequest(BaseModel):
    ...
    load_user: bool = Field(
        default=True, description="Load user skills from ~/.agentrt/skills/"
    )
...
def get_skills(request: SkillsRequest, http_request: Request) -> SkillsResponse:
    ...
    3. User skills - From ~/.agentrt/skills/
    4. Organization skills - From {org}/.agentrt or equivalent
    5. Project skills (highest) - From {workspace}/.agentrt/skills/
...
def install_skill_endpoint(request: InstallSkillRequest) -> InstalledSkillResponse:
    ...
    the user's installed skills directory (~/.agentrt/skills/installed/).
...
def list_installed_skills_endpoint() -> InstalledSkillsListResponse:
    ...
    directory (~/.agentrt/skills/installed/).
