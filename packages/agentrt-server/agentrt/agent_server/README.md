...
uv run python -m agentrt.agent_server

# Start with custom host and port
uv run python -m agentrt.agent_server --host localhost --port 3000

# Start with auto-reload (for dev)
uv run python -m agentrt.agent_server --reload
```
...
```bash
pip install 'agentrt-server[posthog]'
```
...
# Run with coverage
uv run pytest tests/agent_server/ --cov=agentrt.agent_server
```
