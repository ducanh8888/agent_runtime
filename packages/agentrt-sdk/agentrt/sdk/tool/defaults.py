...
the canonical defaults here lets ``agentrt-sdk`` (which must not import
``agentrt-tools``) default a toolset from data alone — ``Tool`` is a spec
(name + params) resolved to an implementation only at runtime via the registry.

``agentrt.tools.preset.default.get_default_tools`` remains the constructor
