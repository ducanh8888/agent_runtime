...
- ``agentrt-tools`` for command splitting and escape doubling
  (``terminal.utils.command``).
- ``agentrt-sdk`` for AST-aware security detection
  (the planned ``security.defense_in_depth.*`` analyzers).

Hosting the parser inside ``agentrt-sdk`` lets both consumers depend
