...
Example (bare guardrail):
    ...
    >>> from agentrt.sdk.security import ToolShieldLLMSecurityAnalyzer
...
Example (with ToolShield tool-specific safety experiences):
    Install the optional extra ``pip install agentrt-sdk[toolshield]``
...
# per the actor's system prompt (see agentrt.sdk.event.llm_convertible.
...
class ToolShieldLLMSecurityAnalyzer(SecurityAnalyzerBase):
    ...
    ``pip install agentrt-sdk[toolshield]`` and pass the rendered
    ...
    safety_experiences: str = Field(
        ...
        description=(
            ...
            "(``pip install agentrt-sdk[toolshield]``). Callers with "
