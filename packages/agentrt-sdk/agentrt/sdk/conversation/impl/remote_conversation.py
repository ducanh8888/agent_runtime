...
from agentrt.sdk.conversation.base import BaseConversation, ConversationStateProtocol
...
from agentrt.sdk.event.types import EventID
...
from agentrt.sdk.observability.laminar import OPERATION_METADATA_KEY, observe
...
from agentrt.sdk.utils.redact import http_error_log_content
from agentrt.sdk.workspace import LocalWorkspace, RemoteWorkspace
...
class RemoteConversation(BaseConversation):
    ...
    def __init__(
        ...
        **_: object,
        ...
        if should_create:
            ...
            from agentrt.sdk.subagent.registry import get_registered_agent_definitions
            from agentrt.sdk.tool.registry import get_tool_module_qualnames
        ...
        ws_timeout = float(os.getenv("AGENTRT_REMOTE_WS_READY_TIMEOUT", "30"))
        if not self._ws_client.wait_until_ready(timeout=ws_timeout):
            if os.getenv("AGENTRT_REMOTE_WS_READY_REQUIRED", "true").lower() in (
                "0",
                "false",
                "no",
            ):
                logger.warning(
                    ...
                    "reconciliation because AGENTRT_REMOTE_WS_READY_REQUIRED "
                    "is false.",
                    ws_timeout,
                    self._id,
                )
    ...
    def update_secrets(self, secrets: Mapping[str, SecretValue]) -> None:
        from agentrt.sdk.secret.secrets import SecretSource
