...
# Anchor for a relative AGENTRT_PERSISTENCE_DIR: captured once so the state tree
...
def _warn_relative_persistence_dir(env_dir: str) -> None:
    ...
    logger.warning(
        "AGENTRT_PERSISTENCE_DIR=%r is relative; anchoring it to %s. Set an absolute path.",
        env_dir,
        _INITIAL_CWD,
    )
...
def get_user_persistence_dir(default: Path | None = None) -> Path:
    ...
    otherwise returns ``default``, or ``~/.agentrt`` when no ``default`` is
    given. ``AGENTRT_PERSISTENCE_DIR`` replaces the ``~/.agentrt`` base directory;
    callers append their usual subdirectories (e.g. ``profiles``,
    ``cache/skills``) to the result.

    ``AGENTRT_PERSISTENCE_DIR`` is expected to be absolute. A relative value is
    ...
    import time, so ``AGENTRT_PERSISTENCE_DIR`` must be set before ``agentrt.sdk``
    is imported; only call-time call sites pick up a later change.
    """
    env_dir = os.environ.get("AGENTRT_PERSISTENCE_DIR")
    if env_dir:
...
def is_local_path_source(source: str) -> bool:
    ...
    ``.agentrt``), host-native absolute paths, Windows absolute paths, and
