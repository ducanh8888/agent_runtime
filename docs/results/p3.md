...
Two things that only that view showed, both now in `../research/friction-log.md`: a session
...
Recorded in `../reference/daemon-behavior.md`; the ones that changed the design:
...
evidence behind both the cost table in `../guides/orchestration.md` and the "pointing at a document is not
enough" finding in `../guides/orchestration.md`. It looks like junk in `list` and is
not.
